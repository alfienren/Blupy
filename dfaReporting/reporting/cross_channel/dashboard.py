...
def merged_channel_data(online, offline, search, social, tmo):
    ...
    offline.set_index('Week', inplace=True)
    ...
    search_impressions = pd.pivot_table(search, index=['Week'], values=['Branded Search Impressions'], aggfunc=np.sum)

    offline_search_impressions = pd.merge(offline, search_impressions, how='left', right_index=True, left_index=True)

    online_offline = online_combined.append(offline_search_impressions)
    ...
    metric_columns = ['Branded Search Impressions', 'Customer Traffic', 'Direct Load Traffic', 'Gross Adds', 'Mobile Visits',
    ...
    for i in weeks:
        aggregated_tmo['Spend'] = np.where(aggregated_tmo['Week'] == i, aggregated_tmo['Spend'] / len(aggregated_tmo[aggregated_tmo['Week'] == i]),
